...
module.exports.play = async(client, message, busqueda) => {
	...
	if (!message.member.voiceChannel) throw new Error("El miembro autor del mensaje no se encuentra actualmente en un canal de voz.");
	if (!busqueda[0] && !client.music[message.guild.id].rep) throw new Error("El miembro autor del mensaje no proporciono un termino de busqueda, y actualmente no se esta reproduciendo nada.")
	...
	if (client.music[message.guild.id].rep) {
		if (!busqueda[0]) throw new Error("El miembro autor del mensaje no proporciono un termino de busqueda.");
		if (busqueda[0].match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/i)) {
			youtube.getInfo(busqueda[0], async (err, vi) => {
				ytsh(vi.title.normalize("NFD").replace(/[\u0300-\u036f]/g, ""), async (err, v) => {
					if (err) throw new Error("Ocurrio un error buscando la cancion de el link proporcionado.")
